...
function ChangeBackground({data, city}) {
  const currentTime = Date.now()/1000
console.log("what i need",city)
  return (
    <div>
        <video className={`background-video z-n1 sun ${currentTime > city.sunset || currentTime < city.sunrise ? "dark" : ""} ${data.weather[0].main === "Clear" ? "bg-display" : "bg-no-display"}`} id="sun" autoPlay loop muted>
    ...
    <video className={`background-video z-n1 rain ${currentTime > city.sunset || currentTime < city.sunrise ? "dark" : ""} ${data.weather[0].main === "Rain" ? "bg-display" : "bg-no-display"}`} id="rain" autoPlay loop muted>
    ...
    <video className={`background-video z-n1 clouds ${currentTime > city.sunset || currentTime < city.sunrise ? "dark" : ""} ${data.weather[0].main === "Clouds" ? "bg-display" : "bg-no-display"}`} id="clouds" autoPlay loop muted>
    ...
    <video className={`background-video z-n1 drizzle ${currentTime > city.sunset || currentTime < city.sunrise ? "dark" : ""} ${data.weather[0].main === "Drizzle" ? "bg-display" : "bg-no-display"}`} id="drizzle" autoPlay loop muted>
